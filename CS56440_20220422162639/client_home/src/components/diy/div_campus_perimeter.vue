<template>
	<div class="diy_details_box diy_div_campus_perimeter">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/campus_perimeter/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/campus_perimeter/set?campus_perimeter_id=' + _this.obj["campus_perimeter_id"], {
								praise_len
							}, (res) => {
								if(res.result){
