<template>
	<div class="diy_details page_flea_market" id="flea_market_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_flea_market'>
							<div_flea_market :obj="obj"></div_flea_market>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_flea_market from "@/components/diy/div_flea_market.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/flea_market/set?flea_market_id=' + obj["flea_market_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "flea_market",
							source_field: "flea_market_id",
							source_id: this.obj.flea_market_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.flea_market_id;
				}
			},
			/**
			 * 初始化收藏状态
