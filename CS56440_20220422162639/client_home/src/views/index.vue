		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/flea_market/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_flea_market = !show_list_flea_market">
		                        <div class="switch_box">
		                            <span v-if="show_list_flea_market"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--跳蚤市场推荐列表组件 -->
		                <list_flea_market v-if="show_list_flea_market" :list="list_flea_market" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_flea_market.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_flea_market" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_flea_market" :key="i" @click="to_details('flea_market',o,'flea_market_id')">
		                            <td v-for="(oj,n) in fields_flea_market">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/campus_perimeter/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_campus_perimeter = !show_list_campus_perimeter">
		                        <div class="switch_box">
		                            <span v-if="show_list_campus_perimeter"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--校园周边推荐列表组件 -->
		                <list_campus_perimeter v-if="show_list_campus_perimeter" :list="list_campus_perimeter" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_campus_perimeter.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_campus_perimeter" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_campus_perimeter" :key="i" @click="to_details('campus_perimeter',o,'campus_perimeter_id')">
		                            <td v-for="(oj,n) in fields_campus_perimeter">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_flea_market from "@/components/diy/list_flea_market.vue";
	import list_campus_perimeter from "@/components/diy/list_campus_perimeter.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_campus_perimeter: true,
				list_campus_perimeter: [],
				vm_arr_campus_perimeter: [
					"campus_perimeter_id",
					"store_name",
					"store_address",
					"store_telephone",
		...
		methods: {
			...
			// 获取跳蚤市场列表
			get_flea_market() {
				let url = "~/api/flea_market/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_flea_market = json.result.list;
					}
				})
			},
			// 获取校园周边列表
			get_campus_perimeter() {
				let url = "~/api/campus_perimeter/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_campus_perimeter = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_flea_market();
			this.get_campus_perimeter();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_flea_market() {
		        var list = this.list_flea_market;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_flea_market.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_campus_perimeter() {
		        var list = this.list_campus_perimeter;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_campus_perimeter.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
