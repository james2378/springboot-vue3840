<template>
	<div class="diy_details page_campus_perimeter" id="campus_perimeter_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_campus_perimeter'>
							<div_campus_perimeter :obj="obj"></div_campus_perimeter>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_campus_perimeter from "@/components/diy/div_campus_perimeter.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "campus_perimeter_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/campus_perimeter/set?campus_perimeter_id=' + obj["campus_perimeter_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "campus_perimeter",
							source_field: "campus_perimeter_id",
							source_id: this.obj.campus_perimeter_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.campus_perimeter_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "campus_perimeter",
					source_field: "campus_perimeter_id",
					source_id: obj["campus_perimeter_id"],
					user_id
				};
